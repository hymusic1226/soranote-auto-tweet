...
【可読性ルール・必ず守る】（リプ獲得のため重要）
- 1行を長くしすぎず、意味の塊ごとに空行を入れて段落化する
- 対比やNG/OKを示すときは ❌ と ⭕️ の絵文字で行頭マークする（❌⭕️は絵文字カウント対象外）
- 最後の1行は必ず読者への問いかけ or 共感誘発の一言で終える
  例:「あなたの台本、冒頭何秒で始まる？」「経験ある人いる？」「これ、わかる人いるかな」

【参考レイアウト（この形を真似る）】
制作進行で数々のASMRや声優の配信現場を見てきたけど、台本の冒頭で「今から〇〇するよ」という説明的なセリフが多すぎる。

本当にリスナーに刺さるのは、物理的な距離感や環境音への言及。

❌「今から〇〇するよ」
⭕️「（マイクを少し近づけ）…ん、聞こえてる？」

宅録設定なら「このマイク、ノイズ少なくて良いですね」とかの方が圧倒的にリアル。
...
def sanitize(text: str, pattern_key: str = "") -> str:
    ...
    # 問いかけ保険：末尾が？/?で終わっていなければ汎用の問いかけを追加
    QUESTION_HOOKS = [
        "あなたはどう書いてる？",
        "経験ある人いる？",
        "これ、わかる人いるかな。",
    ...
    if body and body[-1] not in ('？', '?', '。'):
        # 文末記号無し → 文として未完なので問いかけは追加しない
        pass
    elif body and body[-1] not in ('？', '?'):
        # 「。」で終わっている → 問いかけを追加
        body = body + "\n\n" + random.choice(QUESTION_HOOKS)
